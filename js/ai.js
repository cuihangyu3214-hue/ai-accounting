...
要求：
- note 是精炼的摘要，保留关键信息（餐次、物品名、地点等），去掉语气词和废话
- 示例："中午喝胡辣汤花了18" → "午餐胡辣汤"
- 示例："早上买了杯咖啡15块" → "早上咖啡"
- 示例："打车去公司花了30" → "打车去公司"
- 示例："在超市买了水果和零食68" → "超市水果零食"
- note 控制在 2-8 个字
...
要求：提到总支出、预算使用率、占比最高的分类、与上月变化、结余预测。语气友好简洁。`;
  ...
  _fallbackParse(text) {
    ...
    // 精炼摘要：去掉金额、语气词，保留餐次和关键词
    const desc = text.replace(amountMatch[0], '')
      .replace(/[元块钱￥¥]/g, '')
      .replace(/今天|昨天|刚才|刚刚|花了|花的|用了|了$|的$|吃了|买了|坐了|打了/g, '')
      .replace(/上午/g, '早上').replace(/下午/g, '下午')
      .replace(/中午/g, '午餐').replace(/晚上/g, '晚餐').replace(/早上/g, '早餐')
      .trim();
